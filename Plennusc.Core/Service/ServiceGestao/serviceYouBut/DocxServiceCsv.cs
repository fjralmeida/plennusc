...
namespace Plennusc.Core.Service.ServiceGestao.serviceYouBut
{
    public class DocxServiceCsv
    {
     ...
     Dictionary<string, string> dados)
        ...
        /// <summary>
        /// Marca com X a linha da tabela cujo código do plano (primeira coluna) é igual a codigoPlano.
        /// </summary>
        /// <summary>
        /// Marca com X AZUL e NEGRITO a linha da tabela cujo código do plano (primeira coluna) é igual a codigoPlano.
        /// Remove o código original e coloca um X estilizado.
        /// </summary>
        /// <summary>
        /// Marca com X AZUL, NEGRITO e CENTRALIZADO a linha da tabela cujo código do plano (primeira coluna) é igual a codigoPlano.
        /// </summary>
        public int FillPlanoCoparticipacao(string documentoPreenchidoPath, string codigoPlano)
        {
            int marcados = 0;
            using (var doc = WordprocessingDocument.Open(documentoPreenchidoPath, true))
            {
                ...
                foreach (var table in tables)
                {
                    ...
                    for (int i = 1; i < rows.Count; i++)
                    {
                        ...
                        string textoPrimeiraCelula = string.Join("", cells[0].Descendants<Text>().Select(t => t.Text)).Trim();

                        if (string.Equals(textoPrimeiraCelula, codigoPlano, StringComparison.OrdinalIgnoreCase))
                        {
                            // Limpa a célula
                            cells[0].RemoveAllChildren();

                            // Cria parágrafo com alinhamento centralizado
                            Paragraph paragraph = new Paragraph();
                            paragraph.ParagraphProperties = new ParagraphProperties(
                                new Justification() { Val = JustificationValues.Center }
                            );

                            // Run com propriedades: Negrito e Cor Azul
                            Run run = new Run();
                            RunProperties runProperties = new RunProperties();
                            runProperties.Append(new Bold());
                            runProperties.Append(new Color() { Val = "0000FF" });
                            run.RunProperties = runProperties;
                            run.AppendChild(new Text("X"));

                            paragraph.AppendChild(run);
                            cells[0].AppendChild(paragraph);

                            // Centralizar verticalmente a célula
                            TableCellProperties cellProps = cells[0].TableCellProperties ?? new TableCellProperties();
                            cellProps.AppendChild(new TableCellVerticalAlignment() { Val = TableVerticalAlignmentValues.Center });
                            cells[0].TableCellProperties = cellProps;
        ...
        /// <summary>
        /// Extrai texto de uma célula, ignorando símbolos de checkbox (□ ☐) e espaços extras.
        /// </summary>
        private string ExtrairTextoLimpo(TableCell cell)
        {
            string texto = string.Join("", cell.Descendants<Text>().Select(t => t.Text));
            // Remove caracteres de checkbox comuns
            texto = texto.Replace("□", "").Replace("☐", "").Replace("☒", "").Replace("✔", "");
            return texto.Trim();
        }

        /// <summary>
        /// Normaliza: remove acentos, converte para maiúsculo, remove espaços duplicados.
        /// </summary>
        private string NormalizarTexto(string texto)
        {
            ...
            texto = System.Text.RegularExpressions.Regex.Replace(texto, @"\s+", " "); // múltiplos espaços vira um
            return texto.Trim();
        }
            ...
            { "TELEFONE CELULAR",     "TELEFONE_CELULAR" }
                ...
                foreach (var para in paragraphs)
                {
                    ...
                    foreach (var kvp in mapeamento)
                    {
                        ...
                        string chave = kvp.Value;

                        if (textoPara.IndexOf(rotulo, StringComparison.OrdinalIgnoreCase) >= 0)
                        {
