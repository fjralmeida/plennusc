...
namespace Plennusc.Core.SqlQueries.SqlQueriesMedic.interview
{
    ...
    {
        public DataTable GetInterview(string status, string empresa, string plano, string busca)
        {
            string sql = @"
        SELECT 
            CODIGO_ASSOCIADO,
        ...
        WHERE 1=1";
            ...
            if (!string.IsNullOrEmpty(empresa)) parametros.Add("@EMPRESA", empresa);
            if (!string.IsNullOrEmpty(plano)) parametros.Add("@PLANO", plano);
